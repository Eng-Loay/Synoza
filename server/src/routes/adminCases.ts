...
import { caseToForm, formToCaseData, type CaseFormPayload } from '../services/caseFormService.js';
import { importedCaseToForm, parseImportedCaseSource } from '../lib/caseImportParser.js';

const router = Router();
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const examMediaRoot = path.resolve(__dirname, '../../../client/public/exam/cases');
...
router.post('/import/parse', async (req, res) => {
  ...
  if (!source) return res.status(400).json({ error: 'Paste a case object first.' });

  try {
    const data = parseImportedCaseSource(source);
    const [specialties, difficulties, categories] = await Promise.all([
      prisma.specialty.findMany({ where: { isActive: true }, orderBy: { nameEn: 'asc' } }),
    ...
    res.status(400).json({
      error: error instanceof Error ? error.message : 'Could not parse the case object.',
    });
  }
});
...
router.post('/media/upload', async (req, res) => {
  ...
  try {
    const saved = await saveExamMediaFile({ fileName, mimeType, dataBase64, caseSlug });
...
router.post('/', async (req, res) => {
  ...
  if (!form.titleEn?.trim() || !form.specialtyId || !form.difficultyId) {
    return res.status(400).json({ error: 'titleEn, specialtyId, and difficultyId are required' });
  }
