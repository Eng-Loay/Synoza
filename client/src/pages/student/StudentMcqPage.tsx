...
function TermCard({
  ...
  const content = (
    <>
      ...
      <p className="mt-2.5 font-bold text-[15px] leading-snug" style={{ color: QBANK.heading }}>
        {title}
      </p>
      ...
      <div className="grid grid-cols-2 gap-3 mt-5">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <p className="text-xs leading-tight" style={{ color: QBANK.muted }}>
            <span className="font-bold text-[13px]" style={{ color: QBANK.heading }}>
              {term.modules}
            </span>{' '}
            {t('portalMcqModules')}
          </p>
      ...
      <div className="mt-5">
        <div className="flex items-center justify-between gap-3 mb-2">
          <span className="text-xs font-medium" style={{ color: QBANK.muted }}>
            {t('portalMcqProgress')}
          </span>
          {term.enabled ? (
            <span
        ...
        <div className="h-2 rounded-full overflow-hidden" style={{ backgroundColor: QBANK.progressTrack }}>
          <div
  ...
  const cardClass =
    'block text-start rounded-2xl border border-slate-200/80 bg-white p-5 sm:p-6 transition-all shadow-[0_4px_20px_rgba(0,0,0,0.05)]';

  if (term.enabled) {
    return (
      <Link
        to={`/student/mcq/${term.id}`}
        className={`${cardClass} hover:border-[#C7C2FF] hover:shadow-[0_4px_24px_rgba(99,91,255,0.12)]`}
      >
        {content}
      </Link>
    );
    ...
    <div className="max-w-[1100px] mx-auto space-y-10 pb-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 pt-1">
        <div className="max-w-lg">
          <h1 className="text-[2rem] sm:text-[2.5rem] font-bold leading-tight tracking-tight" style={{ color: QBANK.heading }}>
            {t('portalMcqTitle')}
          </h1>
          <p className="mt-2.5 text-[15px] leading-relaxed" style={{ color: QBANK.muted }}>
      ...
      <section>
        <h2 className="text-base font-bold mb-4" style={{ color: QBANK.heading }}>
        ...
        {FEATURES.map(({ icon, titleKey, descKey, iconBg, iconColor }) => (
          <div key={titleKey} className="flex gap-3 items-start">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs leading-relaxed" style={{ color: QBANK.muted }}>
                {t(descKey)}
              </p>
            </div>
          </div>
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-[0_4px_20px_rgba(0,0,0,0.05)]">
        <h2 className="text-base font-bold mb-8" style={{ color: QBANK.heading }}>
          ...
          {STEPS.map(({ icon, titleKey, descKey, iconBg, iconColor }, index) => (
            <div key={titleKey} className="relative text-center lg:text-start">
              {index < STEPS.length - 1 && (
                <div
                  className="hidden lg:block absolute top-5 start-[calc(50%+28px)] w-[calc(100%-56px)] border-t-2 border-dashed border-slate-200"
                  aria-hidden
                />
              )}
      ...
      <p className="text-center text-xs sm:text-sm pt-2 pb-4" style={{ color: QBANK.muted }}>
        {t('portalMcqFooter')}
      </p>
    </div>
  );
